...
with st.sidebar:
    ...
    st.markdown("### 新工具示例问题")
    st.code("我是 P6，司龄 4 年，今年已经用了 3.5 天年假，还剩多少？")
    st.code("我周末加班 5 小时，餐补能领多少？")
    st.code("帮我安排一个 45 分钟、8 人参加的项目复盘会议议程。")
...
def format_tool_call_message(tool_call: ResponseFunctionToolCall) -> str:
    """将工具调用信息格式化为便于查看的 Markdown 文本。"""
...
def format_tool_output_message(tool_output) -> str:
    """将工具执行结果格式化为 Markdown 文本。"""

    if isinstance(tool_output, str):
        formatted_output = tool_output
    else:
        formatted_output = json.dumps(tool_output, ensure_ascii=False, indent=2)
    return f"\n\n```json\n[ToolOutput]\n{formatted_output}\n```\n"
                    ...
                    async def stream_output():
                        ...
                        async for event_type, chunk in response_generator:
                            ...
                            if event_type == "argument":
                                accumulated_text += format_tool_call_message(chunk)
                                placeholder.markdown(accumulated_text + "▌")

                            elif event_type == "raw":
                                accumulated_text += format_tool_output_message(chunk)
                                placeholder.markdown(accumulated_text + "▌")
